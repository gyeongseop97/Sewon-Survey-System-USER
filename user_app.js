...
// ---- session handoff from portal (URL hash) ----
async function ingestPortalSession(){
  try{
    const h = window.location.hash || "";
    if (!h.includes("access_token=") || !h.includes("refresh_token=")) return;
...
async function loadMyDraftForCurrentSurvey() {
  ...
  const { data, error } = await sb
    .from("responses")
    .select("draft_json, submitted_json, submitted_at")
  ...
  // 제출본이 있으면 제출본 우선(원하시면 여기서 편집 잠금도 가능)
...
async function saveDraftToServer() {
  if (!state.server?.id) return alert("먼저 설문 코드를 불러오세요.");
  const session = await requireLoginOrModal();
  const uid = session?.user?.id;
  if (!uid) return;
    ...
    .upsert(
      {
        ...
        respondent_email: session?.user?.email || null,
        survey_id: state.server.id,
        draft_json: null, // 제출 시 draft는 비우기(원하면 유지해도 됨)
...
document.addEventListener("DOMContentLoaded", () => {
  (async () => { await ingestPortalSession(); })();
