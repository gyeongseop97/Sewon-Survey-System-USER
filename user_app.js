...
function renderSubmitNotice(){
  ...
  const ovEl = document.getElementById("userOverview");
  ...
  if (!box){
    ...
    // ✅ insertBefore 기준 노드와 parent가 다르면 오류가 나므로,
    //    항상 "기준 노드의 parent"에 붙입니다.
    if (ovEl && ovEl.parentNode){
      ovEl.parentNode.insertBefore(box, ovEl.nextSibling);
    } else if (headerEl && headerEl.parentNode){
      headerEl.parentNode.insertBefore(box, headerEl.nextSibling);
    } else {
...
async function loadSurveyByCode(){
  ...
  const row = data[0];
  state.server = { id: row.id, code: row.code };
...
`;
    ...
    qs.forEach((q) => {
      ...
      if (c) {
        renderControls(c, q, ans, {
          disabled: (disabledSet.has(q.id) || isEditingLocked()),
          onAnswerChanged: () => handleAnswerChanged(),
        });
      }
    });
...
function serializeAnswersForServer() {
  ...
  for (const [qid, a] of Object.entries(state.answers || {})) {
    answers[qid] = {
      norm: a?.norm || "",
      checks: Array.from(a?.checks || []), // Set -> Array
      text: a?.text || "",
      fields: a?.fields || {},
    };
  }
  return {
    target: { ...state.target }, // company/name 등
    answers,
...
async function loadMyDraftForCurrentSurvey() {
  ...
  const { data, error } = await sb
    .from("responses")
    .select("draft_json, submitted_json, submitted_at, status")
