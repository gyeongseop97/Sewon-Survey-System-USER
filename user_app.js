...
function isEditingLocked(){
  return !!state?.locked || isBeforeOpenNow() || isExpiredNow();
}
...
if (before) {
  ...
  box.textContent = `아직 설문 작성 기간이 시작되지 않았습니다. 오픈 예정 시각: ${fmtKST(state?.surveyWindow?.open_from)}`;
  return;
}
...
// ✅ 시작 전이면 설문 자체를 렌더링하지 않음
if (!w.ok && w.reason === "before") {
  state.survey = { title: "", version: "", schemaVersion: 2, groups1: [], rules: [] };
  state.answers = {};
  state.g2Index = [];
  state.currentG2Id = null;
  ...
  alert(`아직 설문 작성 기간이 시작되지 않았습니다.\n\n오픈 예정 시각: ${fmtKST(state.surveyWindow?.open_from)}`);
  return;
}
